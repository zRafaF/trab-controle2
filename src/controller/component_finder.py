...
cap_decades = [1e-9, 1e-8, 1e-7, 1e-6, 1e-5, 1e-4]  # nF to uF
res_decades = [1e2, 1e3, 1e4, 1e5, 1e6]  # 100 Ohm to 1 MOhm

# Generate component value lists
capacitors_e6 = generate_e_series(E6_BASES, cap_decades)
capacitors_e12 = generate_e_series(E12_BASES, cap_decades)
resistors_e12 = generate_e_series(E12_BASES, res_decades)
resistors_e24 = generate_e_series(E24_BASES, res_decades)
...
scenarios = [
    {
        "name": "R: E24, C: E12",
        "resistors": resistors_e24,
        "capacitors": capacitors_e12,
    },
    {
        "name": "R: E12, C: E12",
        "resistors": resistors_e12,
        "capacitors": capacitors_e12,
    },
...
print("--- Searching for Optimal Commercial Components Across Scenarios ---")
...
for scenario in scenarios:
    ...
    print(
        f"R1: {best_scenario_components['R1']/1e3:.2f} kΩ, R2: {best_scenario_components['R2']/1e3:.2f} kΩ, C1: {best_scenario_components['C1']*1e6:.2f} µF, C2: {best_scenario_components['C2']*1e6:.2f} µF"
    )
    print(
        f"Achieved Gains: Kp={best_scenario_components['kp_actual']:.3f}, Ki={best_scenario_components['ki_actual']:.3f}, Kd={best_scenario_components['kd_actual']:.3f}"
    )
...
print("--- Overall Optimal Component Values ---")
...
pv_actual, co_actual = run_simulation(
    -overall_best_components["kp_actual"],
    -overall_best_components["ki_actual"],
    -overall_best_components["kd_actual"],
)

# Plotting the comparison
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 12), sharex=True)
fig.suptitle(
    f"System Response: Ideal vs. Best Achievable ({overall_best_components['scenario_name']})",
    fontsize=16,
)
